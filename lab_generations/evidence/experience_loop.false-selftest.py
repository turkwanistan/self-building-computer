...
def selftest()->dict[str,Any]:
    ...
    src=pathlib.Path(__file__).read_text(); a=src.index("retrieval=memory.retrieve(task)"); b=src.index("matches=forge.search(gap)"); ck("memory_before_forge",a<b)
    ck("no_open_gap_in_plan","open_gap(" not in src)
    ck("no_permanent_mcp_dependency","mcp.tool" not in src)
